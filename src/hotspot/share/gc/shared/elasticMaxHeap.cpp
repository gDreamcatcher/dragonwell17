...
void PS_ElasticMaxHeapOp::doit() {
  ...
  size_t new_young_limit = align_down_bounded(_new_max_heap / (NewRatio + 1), GenAlignment);
...
void G1_ElasticMaxHeapOp::doit() {
  ...
  if (is_shrink) {
    ...
    if (!g1_can_shrink(_new_max_heap, maximum_used_percentage, max_heap_size)) {
      // trigger Young GC
      policy->collector_state()->set_in_young_only_phase(true);
      policy->collector_state()->set_in_full_gc(false);
      GCCauseSetter gccs(heap, _gc_cause);
...
bool CollectedHeap::update_elastic_max_heap(size_t new_size, outputStream* st, bool init_shrink) {
  st->print_cr("update_elastic_max_heap: %lu, new_size", new_size);
  new_size = align_up(new_size, HeapAlignment);
  if (new_size > MaxHeapSize) {
...
bool PS_ElasticMaxHeapOp::ps_old_gen_can_shrink(size_t new_limit, bool print_logs) {
  ...
  min_free = align_up(min_free, heap->old_gen()->virtual_space()->alignment());
...
bool PSYoungGen::shrink_after_full_gc(size_t new_size) {
  ...
  ParallelScavengeHeap* heap = (ParallelScavengeHeap*)Universe::heap();
  size_t space_alignment = SpaceAlignment;
...
void G1_ElasticMaxHeapOp::g1_shrink_without_full_gc(size_t _new_max_heap) {
  ...
  heap->verifier()->verify_region_sets_optional();
  heap->verifier()->verify_after_gc(G1HeapVerifier::G1VerifyFull);
  heap->uncommit_regions_if_necessary();
